...
최신 업데이트에서 추가된 출석률 대시보드는 다음과 같은 기능을 제공합니다:

1. **실시간 출석률 표시**:
   - 총원 대비 출석한 학생 수와 출석률(%)을 실시간으로 표시합니다.
   - 예: "12/30명 (40%)" 형식으로 표시됩니다.

2. **시각적 프로그레스 바**:
   - 출석률을 직관적으로 확인할 수 있는 프로그레스 바가 추가되었습니다.
   - 출석률에 따라 프로그레스 바의 길이가 자동으로 조정됩니다.

3. **자동 업데이트**:
   - 학생이 출석 체크할 때마다 출석률 정보가 자동으로 업데이트됩니다.
   - 새로고침 없이도 실시간으로 출석 현황을 파악할 수 있습니다.

4. **테이블 헤더 통계**:
   - 학생 출석 현황 테이블 헤더에도 출석 학생 수/총 학생 수 정보가 표시됩니다.

이 기능은 선생님이 수업 중에 한눈에 출석 현황을 파악할 수 있게 해주고, 출석 독려를 위한 정보를 제공합니다.
